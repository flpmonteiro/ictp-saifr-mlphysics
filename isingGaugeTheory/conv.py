...
Nord=1 # number of temperatures in the ordered phase
...
x = tf.placeholder("float", shape=[None, 2*lx*lx]) # spin configuration
...
W_1 = weight_variable([2*lx*lx,hiddenunits1])
...
for i in range(Ntemp):
  ...
  for j in range(samples_per_T):
        batch=(mnist.test.images[ii,:].reshape((1,2*lx*lx)),mnist.test.labels[ii,:].reshape((1,numberlabels)))     
...
for ii in range(Ntemp):
  batch=(mnist.test.images[ii*samples_per_T:ii*samples_per_T+samples_per_T,:].reshape(samples_per_T,2*lx*lx), mnist.test.labels[ii*samples_per_T:ii*samples_per_T+samples_per_T,:].reshape((samples_per_T,numberlabels)) )
  train_accuracy = sess.run(accuracy,feed_dict={
        x:batch[0], y_: batch[1]}) 
...
f = open('hidden_input.dat', 'w')
for ii in range(Ntemp*samples_per_T):
 batch=(mnist.test.images[ii,:].reshape(1,2*lx*lx), mnist.test.labels[ii,:].reshape((1,numberlabels)))
 mag=np.sum((2*batch[0]-1 ))/(2*lx*lx)
 output=sess.run(hl,feed_dict={x:batch[0]})
 f.write(str(mag)+'  '+' '.join(map(str, output[0,:]))+"\n")
