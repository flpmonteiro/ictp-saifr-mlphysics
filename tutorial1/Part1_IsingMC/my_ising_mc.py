...
import numpy as np
import random # TODO: random is thread safe, np.random is not but is faster. its possible to make np.random threadsafe?
import argparse
...
parser.add_argument('-eq', metavar='', type=int, default=100, help='number of equilibration sweeps')
...
Tc = 2.0/np.log(1.0 + np.sqrt(2))*J # analytical value of critical temperature
...
def neighbours(i,j):
    ...
    neigh = np.zeros((N_spins,4),dtype=np.int)
    for i in range(N_spins):
